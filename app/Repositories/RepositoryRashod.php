    ...
    public function SelectRashod($type,$priznak)
    {
      ...
      foreach($ostatki AS $ostatks)
      {
          ...
          foreach($ostatkNominalDistinct AS $distinctNaminal):
             ...
             $ostatkiResult= ostatki_safe::select('cell_id','id','safe_id','shkaf_id','qator_id','ed_id','naminal','summa')->where('naminal',$distinctNaminal['naminal'])->where('cell_id',$ostatks['cell_id'])->where('type',$type)->where('priznak',$priznak)->orderBy('id','desc')->limit(1)->get();
