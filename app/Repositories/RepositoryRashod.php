...
class RepositoryRashod{
    ...
    public function SelectRashod($type,$priznak)
    {
      ...
      $keys = array_column($arrayResult, 'safe_id');
      array_multisort($keys, SORT_ASC, $arrayResult);
        array_map("unserialize", array_unique(array_map("serialize", $arrayResult)));
    ...
    public function SelectRashodAll($type,$priznak)
    {
      ...
      foreach($ostatki AS $ostatks)
      {
          ...
          foreach($ostatkNominalDistinct AS $distinctNaminal):
             ...
             $ostatkiResult= ostatki_safe::select('cell_id','id','safe_id','shkaf_id','qator_id','ed_id','naminal','summa','typeFond')->where('naminal',$distinctNaminal['naminal'])->where('cell_id',$ostatks['cell_id'])->where('priznak',$priznak)->orderBy('id','desc')->limit(1)->get();
      ...
      public function InsertRashodBotilshudaToOstatkiTanga($request)
      {
        ...
        foreach($request['id'] AS $input)
        {
            ...
            // exit;
               ...
               //  print_r(json_decode($ostatki_safe,true));
