 ...
 $(document).on("click",'#priznak',function(){
    //  alert($(this).val());
    // document.getElementById("calcForms").reset();
 
 
     var src= '<input type="hidden" name="src" value="7">';
     var src1far= '<input type="hidden" name="src" value="3">';
     var src1= '<label for="schet">Счет	</label> <div class="btn-group"> <input type="radio" class="btn-check" name="src" id="option1" autocomplete="off"  value="7" required /><label class="btn btn-outline-secondary" for="option1">Оборот</label> <input type="radio" class="btn-check" name="src" id="option3"  value="2" autocomplete="off" required /><label class="btn btn-outline-secondary" for="option2">Фарсуда</label> </div>';
     var srcfar= '<label for="schet">Счет	</label> <div class="btn-group"> <input type="radio" class="btn-check" name="src" id="option2" autocomplete="off"  required value="7" /><label class="btn btn-outline-secondary" for="option1">Оборот</label> <input type="radio" required class="btn-check" name="src" id="option4"  value="1" autocomplete="off" /><label class="btn btn-outline-secondary" for="option2">Коршоям</label> </div>';
     
     var srcbot1= '<label for="schet">Счет</label> <div class="btn-group"> <input type="radio" class="btn-check" name="src" id="option3" autocomplete="off"  required value="7" /><label class="btn btn-outline-secondary" for="option1">Душанбе</label> <input type="radio" required class="btn-check" name="src" id="option5"  value="1" autocomplete="off" /><label class="btn btn-outline-secondary" for="option2">Нобудкуни</label> </div>';
...
$(document).on("click",'#flexSwitchCheckChecked',function(){
  // $safe_id = $("#safe_id01 > option").clone();
   var  $safe_id=  $('#safe_id01').html();
// console.log($('#safe_id01').html());
      ...
      var safe_select='<select name="safe_id'+$(this).val()+'s"   id="safe_id'+$(this).val().substr(-1)+'s" class="form-control selects01s Somon"></select>';
      var shaving_select='<select name="shaving'+$(this).val()+'s"   id="shaving'+$(this).val().substr(-1)+'s" class="form-control selects01'+$(this).val().substr(-1)+'1 Somon"> <option   selected value="">Выберите </option></select>';
